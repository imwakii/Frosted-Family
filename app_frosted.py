...
SLOT_COLORS  = {"CORE": "#27AE60", "SUB": "#E67E22", "TUNDRA": "#95A5A6"}
...
with st.sidebar:
    ...
    sel_slots = st.multiselect(
        "CWL Slot", ["CORE", "SUB", "TUNDRA"], default=["CORE", "SUB"]
    )
...
with tab_ros:
    for clan, icon, fmt in [
        ...
        # Sort: CORE first (by score), then SUB (by score), then TUNDRA
        ...
        with st.expander(
            f"{icon} Frosted {clan} — {fmt} · core avg **{avg:.1f}** · "
